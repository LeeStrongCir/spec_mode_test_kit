...
# and matches the resolved active FEATURE_DIR (so /speckit-test.plan can skip git branch pattern checks).
...
get_feature_paths() {
    ...
    #   2. .specify/feature.json "feature_directory" key (persisted by /speckit-test.specify)
