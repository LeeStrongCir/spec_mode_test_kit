...
<!-- 
  ...
  任务 MUST 按四阶段生命周期组织：
  - 阶段1：测试用例编写、阶段2：测试自动化代码编写、阶段4：测试执行 — 内部任务可并行 [P]
  - 阶段间严格串行依赖：阶段1 → 阶段2 → 阶段3 → 阶段4
...
- [ ] T006 [P] [CASE-SC-01] 编写 SC-01 手工测试步骤在 `docs/test-scenarios/sc-01.md`
- [ ] T007 [P] [CASE-SC-01] 定义 SC-01 预期输出和验证点在 `docs/test-scenarios/sc-01.md`

### 阶段 1.2: SC-02 用例编写

**目标**：[简述该场景的测试目的，如"验证批量 LECS 实例规格变更的并发处理"]

- [ ] T008 [P] [CASE-SC-02] 编写 SC-02 手工测试步骤在 `docs/test-scenarios/sc-02.md`
- [ ] T009 [P] [CASE-SC-02] 定义 SC-02 预期输出和验证点在 `docs/test-scenarios/sc-02.md`
...
- [ ] T018 [P] [EVT-SC-01] 执行回归验证，输出测试总结报告在 `tests/reports/summary.md`
...
1. 完成 阶段 1: P0 场景的测试用例编写（内部可并行）
2. 完成 阶段 2: P0 场景的自动化代码编写（内部可并行）
3. 完成 阶段 3: 测试环境准备
4. 执行 阶段 4: P0 场景的测试执行（内部可并行）
